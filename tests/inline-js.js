...
test('inlines modules smaller than passed "max" option', async (t) => {
  t.is(
    await inlineJs(remoteInclude, {max: 20000}),
...
test('keeps CDN tag for modules larger than "max" option', async (t) => {
  t.is(await inlineJs(remoteInclude, {max: 10}), remoteInclude);
});

const relativeScriptInclude = `<script src="./reflect.js"></script>`;

test('keeps relative includes', async (t) => {
  t.is(await inlineJs(relativeScriptInclude), relativeScriptInclude);
});

const sampleInlineScript = `<script>console.log('hello world');</script>`;

test('keeps inline scripts', async (t) => {
  t.is(await inlineJs(sampleInlineScript), sampleInlineScript);
});
